...
// Format the data to match the ProductMachines interface
const formattedMachines = machine_Description.map(machine => ({
  name: machine.name,
  url: '', // You can add URLs later if needed
  metadata: {
...
// Function to test the bulk upload
async function testBulkUpload() {
  try {
    const response = await fetch('http://localhost:3000/api/machines/descriptions', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify(formattedMachines)
    });

    const result = await response.json();
    console.log('Upload Result:', result);
  } catch (error) {
    console.error('Error uploading machines:', error);
  }
}
